...
$pluginIncludePath = array(
    '0' => realpath($filePath.'plugins/shopping/system/app')
);
...
$cacheBackendOptions['cache_dir'] = $websiteConfig['path'] .'plugins'.DIRECTORY_SEPARATOR.'shopping'.DIRECTORY_SEPARATOR.'cronCache'.DIRECTORY_SEPARATOR.'outOfStockCache'.DIRECTORY_SEPARATOR;
...
if (!empty($disableOutOfStock)) {
    $productMapper = Models_Mapper_ProductMapper::getInstance();
    $where = $productMapper->getDbTable()->getAdapter()->quoteInto('inventory = ?', '0');
    $where .= ' AND ' . $productMapper->getDbTable()->getAdapter()->quoteInto('enabled = ?', '1');
    $productsData = $productMapper->fetchAllData($where, null, null, null, true);
    if (!empty($productsData)) {
        foreach ($productsData as $productData) {
            $productModel = $productMapper->find($productData['id']);
