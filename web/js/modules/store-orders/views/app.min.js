...
$(a.currentTarget),c=parseInt(e.closest("div").data("order-id")),d=_.isUndefined(b["Are you sure you want to change status for this order?"])?"Are you sure you want to change status for this order?":b["Are you sure you want to change status for this order?"],f=e.data("status"),h=this.orders.get(c);if("refunded"===f){d=_.isUndefined(b["Are you sure you want to refund this payment?"])?"Are you sure you want to refund this payment?":b["Are you sure you want to refund this payment?"];a=_.isUndefined(b.Refund)?
"Refund":b.Refund;var l={},g=_.template(t,{i18n:b,orderId:c,gateway:h.get("gateway"),total:h.get("total")});l[a]=function(){$(".ui-dialog").css("zIndex","101");smoke.confirm(d,function(a){a&&$.ajax({url:$("#website_url").val()+"plugin/shopping/run/refundPayment/orderId/",type:"POST",dataType:"json",data:{orderId:$("#refund-order-id").val(),refundAmount:$(".partial-refund-amount").val(),refundInfo:$(".refund-info").val(),secureToken:$(".orders-secure-token").val(),paymentGateway:h.get("gateway"),refundUsingPaymentGateway:$("input.use-refund-payment-gateway").is(":checked")?
