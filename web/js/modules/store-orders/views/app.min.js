...
var d=_.isUndefined(b["Do you want to create a label?"])?"Do you want to create a label?":b["Do you want to create a label?"],c=_.isUndefined(b["Do you want to use this date for the shipping label?"])?"Do you want to use this date for the shipping label?":b["Do you want to use this date for the shipping label?"],e=$(a.currentTarget).data("order-id"),f=this,g=this.orders.get(e),h={};a=_.isUndefined(b["Create label"])?"Create label":b["Create label"];h[a]=function(){$(".ui-dialog").css("zIndex","101");
smoke.confirm(c,function(a){a&&(a=$(".shipping-availability-date-option:checked").val(),f.generateShippingLabelRequest(e,a))},{ok:_.isUndefined(b.Yes)?"Yes":b.Yes,cancel:_.isUndefined(b.No)?"No":b.No})};smoke.confirm(d,function(a){if(a)return a=_.template(s,{orderId:e,i18n:b,shippingAvailabilityDays:g.get("shipping_availability_days")}),$(a).dialog({dialogClass:"seotoaster",width:"75%",height:"400",resizable:!1,buttons:h}),checkboxRadioStyle(),!1})},generateShippingLabelRequest:function(a,b){$.ajax({url:$("#website_url").val()+
"plugin/shopping/run/shippingLabel/",type:"POST",dataType:"json",data:{orderId:a,secureToken:$(".orders-secure-token").val(),availabilityDate:b}}).done(function(a){"1"==a.error?showMessage(a.responseText,!0,5E3):(showMessage(a.responseText.message,!1,5E3),model.set({shipping_label_link:a.responseText.shipping_label_link}))})},checkAllOrders:function(a){var b=this.orders.ordersChecked,c=[];a.target.checked?this.orders.each(function(a){b=_.union(b,[a.id]);a.set({checked:!0})}):(this.orders.each(function(a){c=
_.union(c,[a.id]);a.set({checked:!1})}),b=_.difference(b,c));this.orders.ordersChecked=b;"function"===typeof _checkboxRadio&&_checkboxRadio()},toggleOrder:function(a){var b=$(a.target).val();a.target.checked?this.orders.ordersChecked=_.union(this.orders.ordersChecked,[b]):(a=_.filter(this.orders.ordersChecked,function(a){return a!==b}),this.orders.ordersChecked=a);"function"===typeof _checkboxRadio&&_checkboxRadio()},exportOrdersAction:function(){var a=this.orders.ordersChecked.join(","),d=this.orders.server_api.filter();
_.isUndefined(b.Export);$.ajax({url:$("#website_url").val()+"plugin/shopping/run/getOrderExportConfig/",type:"GET",dataType:"json"}).done(function(c){c=_.template(p,{ordersIds:a,filters:$.param(d),i18n:b,defaultConfig:c.responseText.defaultConfig,exportConfig:c.responseText.export_config});$(c).dialog({dialogClass:"seotoaster",width:"75%",height:"750",resizable:!1});return!1})},resetFilters:function(){this.$("form.filters > :input").val("")},render:function(){},renderOrder:function(a){a.set({useInvoice:$("#invoiceEnable").val()});
...
'system/images/ajax-loader-small.gif" style="margin: 20px auto; display: block;">')},success:function(a){showMessage(_.isUndefined(b.Saved)?"Saved":b.Saved,a.hasOwnProperty("error")&&a.error);!a.error&&a.hasOwnProperty("responseText")&&g.set("status",a.responseText.status)}})},{ok:_.isUndefined(b.Yes)?"Yes":b.Yes,cancel:_.isUndefined(b.No)?"No":b.No})},changeTracking:function(a){var d=$(a.currentTarget),c=parseInt(d.closest("tr").find("td.order-id").text()),e=this.orders.get(c);$.ajax({url:$("#website_url").val()+
...
data:a,type:"POST",dataType:"json",beforeSend:function(){d.closest("td").html('<img src="'+$("#website_url").val()+'system/images/ajax-loader-small.gif" style="margin: 20px auto; display: block;">')},success:function(a){a.hasOwnProperty("error")&&!a.error&&showMessage(_.isUndefined(b.Saved)?"Saved":b.Saved);a.hasOwnProperty("responseText")&&e.set({status:a.responseText.status,shipping_tracking_id:a.responseText.shippingTrackingId})}})})},close:function(a,b){$(this).dialog("close").remove()}})})},
sendInvoice:function(a){a=$(a.currentTarget);var d=parseInt(a.closest("tr").find("td.order-id").text()),c=a.closest("td"),e=c.html();$.ajax({url:$("#website_url").val()+"plugin/invoicetopdf/run/sendInvoiceToUser/",data:{cartId:d,dwn:0},type:"POST",dataType:"json",beforeSend:function(){c.html('<img src="'+$("#website_url").val()+'system/images/ajax-loader-small.gif" style="margin: 20px auto; display: block;">')},success:function(a){a.hasOwnProperty("error")&&(a.error?showMessage(a.responseText,!1,
5E3):showMessage(_.isUndefined(b["Invoice has been sent"])?"Invoice has been sent":b["Invoice has been sent"]));c.html(e)}})},toggleRecurring:function(a){"recurring_id"===$(a.currentTarget).val()?$(".recurring-filters").removeClass("hidden"):$(".recurring-filters").addClass("hidden")}})});
