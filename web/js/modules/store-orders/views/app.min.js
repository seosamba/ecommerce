...
"plugin/shopping/run/fetchNames",type:"GET",dataType:"json"}).done(function(a){console.log(a);a=_.template(q,{data:a.responseText.data,orderId:c,i18n:b});$(a).dialog({width:600,dialogClass:"seotoaster",resizable:!1,open:function(a,f){$(".setTracking").on("click",function(){});$(".save-data").on("click",function(){var a=$("#marketing-services").val(),f=$("#shippingTrackingId").val(),a={name:a,shippingTrackingId:f};$.ajax({url:$("#website_url").val()+"plugin/shopping/run/order?id="+c,data:a,type:"POST",
dataType:"json",beforeSend:function(){d.closest("td").html('<img src="'+$("#website_url").val()+'system/images/ajax-loader-small.gif" style="margin: 20px auto; display: block;">')},success:function(a){a.hasOwnProperty("error")&&!a.error&&showMessage(_.isUndefined(b.Saved)?"Saved":b.Saved);a.hasOwnProperty("responseText")&&e.set({status:a.responseText.status,shipping_tracking_id:a.responseText.shippingTrackingId})}})})},close:function(a,b){$(this).dialog("close").remove()}})})},changeTracking:function(a){var d=
$(a.currentTarget),c=parseInt(d.closest("tr").find("td.order-id").text()),e=this.orders.get(c);if(!e)return!1;smoke.prompt(_.isUndefined(b["Insert tracking code for this order"])?"Insert tracking code for this order":b["Insert tracking code for this order"],function(a){if(!1===a)return a;a=$.trim(a);e.get("shipping_tracking_id")!==a&&$.ajax({url:$("#website_url").val()+"plugin/shopping/run/order?id="+c,data:{shippingTrackingId:a},type:"POST",dataType:"json",beforeSend:function(){d.closest("td").html('<img src="'+
