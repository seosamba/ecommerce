<template>
    <div v-show="loadedScreen" id="assign-product-shipping-dialog" style="width:25%;height:240px">
      <div class="system-popup-header-block">
        <span class="system-popup-title">{{ucFirstAllText($t('message.massActionAssignShipping'))}} ({{$t('message.dialogTitleTotal')}}
      ...
      <div class="system-popup-content-block">
        <form class="grid_12" method="POST" name="assign-product-shipping" id="assign-product-shipping" action="">
          <p class="grid_12">
            <select v-model="selectedShipping" name="shippings" class="shippings">
              <option value="" disabled>{{$t('message.selectProductShipping')}}</option>
              <option v-for="shipping in productShipping" :value="shipping.value">{{$t('message.'+shipping.name)}}</option>
            </select>
          </p>
          <p class="grid_12">
            ...
            <label class="fl-left mt10px pointer matching-filter">{{$t('message.dialogAllProductsMatchingFilter')}}
              <input v-model="matchingFilter" @change.prevent="countProducts($event)" id="assign-price-all-products-matching-filter" class="allShippingFilterProducts" type="checkbox" name="allShippingFilterProducts" value="0"/>
            </label>
...
<script src="./controller/massactiongridassignshipping.js"/>
